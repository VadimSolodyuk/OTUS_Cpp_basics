...
public:
    ...
    explicit listContainer (const T value) : listContainer() {
    ...
    ~listContainer () {
        ...
        delete _end;
        std::cout << "Deleting listContainer" << std::endl; 
    }
    ...
    void insert (const size_t pos, const T value)   { //pos - позиция (по счёту)
        if (pos == 0 || pos > m_sizeList) {
            std::cout <<  R"(Fault "Position out of range!". )"
            << getName() << " contains " << m_sizeList << " elements.\n";
            return;
        }
        Node<T>* new_node = new Node<T>{};
        new_node->data = value;
    ...
    //Поиск нужного Node в зависимости от его удаленности от начала|конца контейнера      
